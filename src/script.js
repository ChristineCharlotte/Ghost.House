...
for (let i = 0; i < 50; i++) {
  ...
  const radius = 3 + Math.random() * 6; //半径
...
const ghost3 = new THREE.PointLight('#ffff00', 5, 3);
...
const tick = () => {
  ...
  // console.log(ghostAngle);
  ghost3.position.x =
    Math.cos(ghost3Angle) * (7 + Math.sin(ghost3Angle) * 0.32);
  ghost3.position.z = Math.sin(ghost3Angle) * (7 + Math.sin(ghost3Angle) * 0.3);
